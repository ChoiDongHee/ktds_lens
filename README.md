...
| `FETCH_CONTENT_TIMEOUT_SECONDS` | 본문 가져오기(trafilatura) 1건당 최대 대기 시간(초) — 기본 30초인 trafilatura 자체 타임아웃 대신 사용 | `8` |
...
| `본문 보강 완료 (Ns)` | 소요시간(N)이 `FETCH_CONTENT_TIMEOUT_SECONDS`에 근접/초과하면 후보 중 하나가 크거나 느린 페이지(PDF 등)였을 가능성이 큼 — 실제로 대형 PDF 링크 하나 때문에 37.9초가 걸린 사례를 발견하고 타임아웃을 8초로 제한한 이력 있음. |
