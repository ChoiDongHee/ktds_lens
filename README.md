...
- **`web_search(query, max_results=3, engines="all", output_format="text")`** — 여러 검색엔진을 동시에 조회해 제목/URL/본문을 엔진별로 묶어서 반환합니다.
  ...
  - **PDF 결과 제외**: URL이 PDF인 결과는 결과 자체에서 완전히 제외합니다 (끄는 옵션 없음). Naver의 검색 스니펫도, 저희가 직접 가져온 본문도 PDF는 문맥 없이 잘린 텍스트라 읽기 어렵다는 걸 실측으로 확인해서 아예 표시하지 않기로 했습니다.
...
```
ktds_lens/
├── server.py                    # MCP 서버 본체 -- 이 저장소의 핵심. 이 파일 하나만 실행하면 서버가 뜸
├── requirements.txt              # pip 의존성 목록
├── .env / .env.example           # 환경설정 (.env는 실제 값이라 git에 안 올라감 / .env.example은 템플릿)
├── search.log, hit_count.log, search_stats.json   # 서버 실행 중 자동 생성되는 런타임 파일 (git에 안 올라감)
│
├── bm25_search/                  # BM25 유사도 검색 패키지 -- server.py가 관련성 필터로 직접 import
│   ├── fetcher.py                 # 네이버 뉴스 후보 목록 수집 (독립 파이프라인 전용)
│   ├── tokenizer.py                # Kiwi 형태소 분석 -- server.py가 관련성 필터에서 재사용
│   ├── bm25_ranker.py               # rank_bm25 래퍼 -- server.py가 관련성 필터에서 재사용
│   ├── ranker.py                    # 최신성 감쇠 재정렬 (독립 파이프라인 전용)
│   └── pipeline.py                  # 위 네 개를 오케스트레이션하는 독립 CLI (python -m bm25_search.pipeline)
│
├── test/                          # server.py의 실제 동작에는 관여하지 않는 테스트/데모 코드
│   ├── test_mcp_client.py          # 실제 MCP 프로토콜 종단 테스트 (서버 프로세스를 띄워 진짜 클라이언트처럼 연결)
│   ├── test_bm25_benchmark.py      # bm25_search 성능/품질 벤치마크
│   └── webapp/app.py                # FastAPI 데모 챗 -- server.py를 모듈로 직접 import, MCP 프로토콜은 안 거침
│
├── backup/google_engine_removed.py  # 제거된 Google 검색 엔진 코드 (복원 노트 포함, 참고용 보관)
├── docs/lens-overview.html         # 전체 설명 문서 (브라우저로 열기)
└── README.md / CLAUDE.md / PLAN.md  # 문서 (사용법 / AI 코딩 어시스턴트 가이드 / 설계·검증 기록)
```

실제 MCP 서버가 참조하는 코드는 **`server.py`와 `bm25_search/` 두 곳뿐**입니다. `test/`, `backup/`, `docs/`는 개발/검증/문서 목적으로만 존재합니다.
...
`.env`를 채운 뒤(아래 [설정](#설정-env) 참고), 세 가지 방식 중 하나로 띄울 수 있습니다. 어느 방식이든 실제로 실행되는 건 동일한 `server.py`이고, 차이는 **"누가 이 프로세스를 실행/유지하는가"** 뿐입니다.
...
### `server.py` 실행 시 내부적으로 일어나는 일 (기동 순서)

1. `.env` 로드 + Windows UTF-8 인코딩 강제 설정 (콘솔 기본 코드페이지로는 검색 결과의 한글/특수문자를 못 담아 프로토콜이 깨지는 걸 방지)
2. 로깅 설정 — `search.log` + stderr로만 기록 (stdout은 MCP 프로토콜 전용이라 절대 안 씀)
3. `CallStats`가 `search_stats.json`을 읽어 오늘자 엔진/키별 호출 수를 복원
4. 엔진 등록 — DuckDuckGo + 네이버(`.env`에 설정된 만큼 뉴스/웹문서/사전) → `SearchRouter` 구성
5. 시작 요약 로그 1줄 기록 (`"Lens 서버 설정: ..."` — 등록된 엔진, `engines="all"`이 실제로 뭘 의미하는지, 관련성 필터/본문보강 on-off 등. [로그 읽는 법](#로그-읽는-법-중요-인사이트) 참고)
6. 호출 현황을 주기적으로 기록하는 백그라운드 데몬 스레드(`_hit_count_loop`) 시작 — 트랜스포트와 무관하게 항상 돎
7. `FastMCP("Lens", ...)` 인스턴스 생성, `web_search`/`search_stats` 두 툴 등록
8. `mcp.run(transport=...)` 호출 — **여기부터가 실제로 "서버로 실행되는" 부분**이며, 이 한 줄의 실제 동작은 트랜스포트에 따라 완전히 다릅니다.

| 트랜스포트 | 실제 동작 | 프로세스를 누가 띄우고 끄는가 |
|---|---|---|
| `stdio` (기본값) | 네트워크 포트를 열지 않음. stdin으로 MCP 요청(JSON-RPC)을 읽고 stdout으로 응답을 씀. | **클라이언트가 직접 실행**합니다 — Claude Desktop/Code가 등록된 명령어로 이 프로세스를 spawn하고, 클라이언트를 끄거나 연결을 끊으면 이 프로세스도 함께 종료됩니다. 사람이 미리 띄워둘 필요가 없습니다. |
| `sse` / `streamable-http` | Uvicorn ASGI 서버가 실제로 `MCP_HOST:MCP_PORT`에 바인딩되어 대기. 클라이언트는 HTTP로 접속. | **사람이 직접 `python server.py`로 띄워서 계속 떠 있게 유지**해야 하는, 일반적인 웹 서버 프로세스와 동일한 형태입니다. 여러 클라이언트가 같은 URL로 동시에 접속할 수 있습니다. |

> **참고**: `test/webapp/app.py`(데모 챗)는 MCP 프로토콜을 전혀 거치지 않습니다 — `import server`로 `server.py`를 그냥 파이썬 모듈로 불러와 `server.router.search()`를 직접 함수 호출합니다. 실제 MCP 프로토콜 종단 테스트가 필요하면 `test/test_mcp_client.py`를 실행하세요 (SSE로 진짜 서버 프로세스를 띄우고 진짜 클라이언트처럼 연결해 검증합니다).
...
| `FETCH_CONTENT_TIMEOUT_SECONDS` | 본문 가져오기(trafilatura) 1건당 최대 대기 시간(초) — 기본 30초인 trafilatura 자체 타임아웃 대신 사용 | `5` |
| `SEARCH_ENGINE_TIMEOUT_SECONDS` | 엔진 하나당 최대 대기 시간(초) | `8` |
...
| `본문 보강 완료 (Ns)` | 소요시간(N)이 `FETCH_CONTENT_TIMEOUT_SECONDS`에 근접/초과하면 후보 중 하나가 크거나 느린 페이지였을 가능성이 큼 — 실제로 대형 PDF 링크 하나 때문에 37.9초가 걸린 사례를 발견한 이력 있음. |
| `PDF 결과 N건 제외` | PDF 링크는 결과 자체에서 완전히 제외됨(URL 확장자로 판별) — Naver 스니펫도, 우리가 직접 가져온 본문도 PDF는 문맥 없는 텍스트 조각이라 읽기 어려운 게 발견되어 아예 결과에서 뺌. |
